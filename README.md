...
export const propertySchema = z.object({
  ...
  breakfast: z.coerce.number().int().min(0, {
...
- components/card
  ...
  - favouriteToggleButton.tsx
  - favouriteToggleForm.tsx
  - LoadingCards.tsx
  - PropertyCard.tsx
  - PropertyRating.tsx
...
components/card/favouriteToggleButton.tsx
...
export const togglefavouriteAction = async () => {
  return { message: 'toggle favourite' };
};
```

### favouriteToggleButton - Complete
...
async function PropertyDetailsPage({ params }: { params: { id: string } }) {
  ...
  const { breakfast, bedrooms, beds, guests } = property;
  const details = { breakfast, bedrooms, beds, guests };
...
function PropertyDetails({
  details: { bedrooms, breakfast, guests, beds },
